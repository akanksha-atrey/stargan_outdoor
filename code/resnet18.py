...
def check_accuracy(mode, model, dataloader, dataset, data_type, testset):
	...
	Arguments:
		...
		data_type (str) : real or fake (generated by stargan)
	...
	with torch.no_grad():
		...
		if testset is None:
			print('%s data on %s: Got %d / %d correct (%.2f)' % (dataset, data_type, num_correct, num_samples, 100 * acc))
		else:
			print('%s data on %s %s: Got %d / %d correct (%.2f)' % (testset, dataset, data_type, num_correct, num_samples, 100 * acc))
	return acc

def test(model_dir, dataloader, dataset='seasonal', data_type='real', testset=None):
	...
	check_accuracy('test', model, dataloader, dataset, data_type, testset)
...
if __name__ == '__main__':
	...
	parser.add_argument('--testset', default=None, help='Name of test set generated by stargan. Can be world_cities, landmarks, transient, both_sl, or both_sc', type=str)
	parser.add_argument('--pretrained', help="If we want a pretrained network or not" , type=bool)
	parser.add_argument('--lr', help='learning rate', type=float)
	parser.add_argument('--epochs', type=int)
	parser.add_argument('--test_data_type', default='real', type=str)
	...
	if config.mode=='train':
		...
		if config.test_data_type == 'real':
			test_data_path = os.path.join(dataset_path[dataset], 'test')
		elif config.test_data_type == 'fake':
			if config.testset is None:
				test_data_path = os.path.join('./results_resnet', dataset)
			else:
				test_data_path = os.path.join('./results_resnet', config.testset)

		test_loader = load_dataset(config.mode, test_data_path)

		trained_model_path = os.path.join(MODEL_PATH, dataset+'.pth')
		test(trained_model_path, test_loader, config.dataset, config.test_data_type, config.testset)
