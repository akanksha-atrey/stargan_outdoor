    ...
    for i, config in enumerate(configurations):
        ...
        while True:
            try:
                ...
                args.extend(['--c_dim', str(7)])
                args.extend(['--rafd_image_dir', 'data/landmarks/train'])
                args.extend(['--sample_dir', 'stargan_landmarks/{}/samples'.format(str(config))])
                args.extend(['--log_dir', 'stargan_landmarks/{}/logs'.format(str(config))])
                args.extend(['--model_save_dir', 'stargan_landmarks/{}/models'.format(str(config))])
                args.extend(['--result_dir', 'stargan_landmarks/{}/results'.format(str(config))])
                ...
                job_name = 'train_sg_landmarks{}'.format(timestamp)
