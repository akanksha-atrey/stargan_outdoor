    ...
    for i, config in enumerate(configurations):
        ...
        while True:
            try:
                ...
                args.extend(['--dataset', 'Both'])
                args.extend(['--c_dim', str(14)])
                args.extend(['--c2_dim', str(7)])
                args.extend(['--celeba_image_dir', 'data/transient_attributes/train'])
                args.extend(['--rafd_image_dir', 'data/landmarks/train'])
                args.extend(['--sample_dir', 'stargan_both_sl/{}/samples'.format(str(config))])
                args.extend(['--log_dir', 'stargan_both_sl/{}/logs'.format(str(config))])
                args.extend(['--model_save_dir', 'stargan_both_sl/{}/models'.format(str(config))])
                args.extend(['--result_dir', 'stargan_both_sl/{}/results'.format(str(config))])
                ...
                job_name = 'train_sg_sl{}'.format(timestamp)
                stdout = log_dir / str(config) / '{}_o.txt'.format(name)
                stderr = log_dir / str(config) / '{}_e.txt'.format(name)
                command = ' '.join(args)
                # queue_name = np.random.choice(['titanx-long','1080ti-long', 'm40-long'], p=[40./total_num_short_jobs, 40./total_num_short_jobs, 12./total_num_short_jobs])
                # queue_name = 'preempt'
